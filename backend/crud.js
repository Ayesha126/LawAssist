...
app.put('/users/:userId', auth, upload.single('profileImage'), async (req, res) => {
  try {
    const userId = parseInt(req.params.userId);
    const { name, contact } = req.body;

    // Verify token (handled by auth middleware, but double-checking here)
    ...
    if (req.file) {
      updateData.profileImage = `/uploads/${req.file.filename}`; // Store the file path
    }
